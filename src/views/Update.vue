<template>
  <div class="container pb-3" :class="darkMode ? 'dark' : ''">
    <div v-if="foundPost && rootIsLoggedIn && rootUserId == cocktailDetails.user_id">
      <h6 class="pt-3">Updating "{{ form.name }}"</h6>
      <hr>
      <ValidationObserver ref="form" v-slot="{ handleSubmit }">
        <b-form @submit.prevent="handleSubmit(onSubmit)" novalidate>
          ...
          <!-- End Form -->
          <hr>
          <b-button type="submit" :variant="darkMode ? 'info' : 'primary'" block>Update!</b-button>
        </b-form>
      </ValidationObserver>
    </div>
    <div v-if="!foundPost || !rootIsLoggedIn">
      <p>Please... </p>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async updateRecipe () {
      ...
      if (response.ok) {
        ...
        setTimeout(() => this.$router.push({ path: `/recipe/${this.recipeId}` }), 1000)
      }
      this.$emit('overlay-control', false)
    },
    assignValue () {
